...
3. **Reuse before generating — and don't trust the node you were handed to be the real thing.** If the Figma component is composed of nested instances of other components, check whether the repo already has each one built. A miss is a decision to name in the report ("this child isn't built yet — building it inline / it needs its own handoff"), not a silent green light to duplicate it.
   ...
   **The harder case, and the one that actually bit a real handoff: a piece that looks like it should be a component — named "Text Input," "Dropdown," styled like a form field or a button — but isn't an `instance` at all, just a `frame`.** Don't conclude "this is ad hoc, I'll build it from what I see" the moment you notice that. Search the file first: `get_metadata` with no `nodeId` lists top-level pages; look for a real component or component set with a matching name or matching composed content. If you find one — even if it has states (Hover, Disabled, Active) the frame you were handed doesn't show — that's the real source, and it's very likely the handed-off node is a detached copy of it. Build against the real component, with its real states, and say plainly in the report that the original node was a detached duplicate and which real component you built from instead. Only fall back to building the frame as-is, states and all, once you've actually searched and found nothing — and say that you searched, not just that nothing turned up.
