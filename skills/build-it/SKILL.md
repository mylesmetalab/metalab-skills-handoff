...
3. **Reuse before generating — and don't trust the node you were handed to be the real thing.** If the Figma component is composed of nested instances of other components, check whether the repo already has each one built. A miss is a decision to name in the report ("this child isn't built yet — building it inline / it needs its own handoff"), not a silent green light to duplicate it.
   ...
   **When reusing an existing child component, use *this instance's* actual props, not that child's generic default.** Figma lets a specific instance override the component it's placed as — a boolean like "show icon" turned off, a specific variant selected — independent of what that component defaults to everywhere else. This has bitten a real build: a button that's correct everywhere else in the file was placed inside this parent with its icon explicitly turned off, and the generated code used the button's *default* (icon on) instead of reading what this specific placement actually specifies. Read the instance's own props for the exact node you're composing (visible in `get_design_context`'s output for the parent, e.g. `<Button showIcon={false} />`) and carry them into the code call site — don't assume a child renders the same way everywhere it's used just because the child component itself is already built correctly.
