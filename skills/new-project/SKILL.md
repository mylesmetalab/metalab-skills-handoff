...
- **`templates/react/`** — the default whenever Node is installed, regardless of whether anyone asked for a build step. React + TypeScript + Vite, with `@base-ui-components/react` already a dependency for anything that needs real accessible behavior (a select, a dialog, a tooltip — see `components/README.md` inside the template), `class-variance-authority` for Figma-variant-to-prop mapping, and CSS Modules against the same `tokens.css` convention as everywhere else. This is the progressive default: typed, componentized, accessible-by-construction, not a step down from what a capable engineer would reach for on their own — because a version of this plugin once shipped a plain-HTML default even when Node was available, and it read as a weaker result purely because of that choice, not because of anything about the design work itself.
